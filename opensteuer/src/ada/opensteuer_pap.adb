...
package body OpenSteuer_PAP is
   ...
   begin
      if VBEZ = 0 then
         ...
         FVB := Cent_0 (Aufrunden_0 (RW_Intern (VBEZ) * 0.4)); -- auf volle Cent aufrunden
         JW := 307200;
         UPANTEIL;
         if FVB > ANTEIL2 then
            FVB := ANTEIL2;
      ...
      if ALTER1 = 0 then
         ...
         BMG := RE4 - VBEZ;
         ALTE := Cent_0 (Aufrunden_0 (RW_Intern (BMG) * 0.4)); -- auf volle Cent aufrunden
         JW := 190800;
         UPANTEIL;
   ...
   begin
      ...
      case STKL is
         when 1 =>
            ANP := 1044;
            SAP := 36;
            KFB := Euro_0 (RW_Intern (ZKF) * 5808.0); -- Test, ob keine Nachkommastellen verloren gehen!
         when 2 =>
            HFB := 2340;
            ANP := 1044;
            SAP := 36;
            KFB := Euro_0 (RW_Intern (ZKF) * 5808.0); -- wie oben!
         when 3 =>
            KZTAB := 2;
            ANP := 1044;
            SAP := 72;
         ...
         when 4 =>
            ANP := 1044;
            SAP := 36;
            KFB := Euro_0 (RW_Intern (ZKF) * 2904.0); -- wie oben!
         when 5 =>
            ANP := 1044;
            KFB := 0;
         when 6 =>
   ...
   begin
      ...
      ZVE := RE4U - ZTABFB - Euro_0 (RW_Intern (VSP)); -- Test, ob VSP keine Nachkommastellen hat!
      MLSTJAHR_2;
   end MLSTJAHR_1;
   ...
   begin
      ...
      VSPMAX1 := Euro_0 (RW_Intern (1334 * Euro_0 (KZTAB)));
      VSPMAX2 := Euro_0 (RW_Intern (667 * Euro_0 (KZTAB)));
      VSPKURZ := Euro_0 (RW_Intern (1134 * Euro_0 (KZTAB)));
      if KRV = 1 then
   ...
   procedure MST5_6 is
   begin
      ZZX := Euro_0 (Abrunden_0 (RW_Intern (X))); -- prüfen, ob X tatsächlich keine Nachkommastelle hier hat!
      if ZZX > 27306 then
         ZX := 27306;
         UP5_6;
   ...
   begin
      case LZZ is
         when 1 =>
            ...
            ANTEIL2 := JW;
         when 2 =>
            ANTEIL1 := Cent_0 (Abrunden_0 (RW_Intern (JW) / 12.0)); -- Ergebnis abrunden.
            ANTEIL2 := Cent_0 (Aufrunden_0 (RW_Intern (JW) / 12.0)); -- Ergebnis aufrunden.
         when 3 =>
            ANTEIL1 := Cent_0 (Abrunden_0 (RW_Intern (JW) * 7.0 / 360.0)); -- Ergebnis abrunden.
            ANTEIL2 := Cent_0 (Aufrunden_0 (RW_Intern (JW) * 7.0 / 360.0)); -- Ergebnis aufrunden.
         when others =>
            ANTEIL1 := Cent_0 (Abrunden_0 (RW_Intern (JW) / 360.0)); -- Ergebnis abrunden.
            ANTEIL2 := Cent_0 (Aufrunden_0 (RW_Intern (JW) / 360.0)); -- Ergebnis aufrunden.
      end case;
   end UPANTEIL;
   ...
   begin
      if SONSTB = 0 then -- Vorsicht Test auf 0!
         ...
         if SONSTB > 15000 then
            ...
            RE4 := JRE4;
            MLSTBER;
            LST1 := Cent_0 (LSTJAHR * 100);
            VBEZ := JVBEZ + VBS;
            RE4 := JRE4 + SONSTB;
            MLSTBER;
            LST2 := Cent_0 (LSTJAHR * 100);
            STS := LST2 - LST1;
         else
            VBEZ := VBEZ + VBS;
            RE4 := RE4 + SONSTB;
         ...
         SOLZS := Cent_0 (Abrunden_0 (RW_Intern (STS) * 5.5 / 100.0)); -- auf volle Cent abrunden.
         if R > 0 then
            BKS := STS;
         else
            BKS := 0;
   ...
   begin
      if VMT = 0 then -- Vorsicht, Test auf 0!
         ...
         RE4 := JRE4 + SONSTB + VMT;
         MLSTBER;
         LST3 := Cent_0 (LSTJAHR * 100);
         LST3 := LST3 - LST1;
         if LST3 < STV then
            STV := LST3;
         end if;
         SOLZV := Cent_0 (Abrunden_0 (RW_Intern (STV) * 5.5 / 100.0)); -- auf volle Cent abrunden;
         if R > 0  then
            BKV := STV;
         else
            BKV := 0;
