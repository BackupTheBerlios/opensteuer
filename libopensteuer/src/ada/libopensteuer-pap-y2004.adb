package body LibOpenSteuer.PAP.Y2004 is
   ...
   begin
      ...
      if RW_Intern (RW_6'First) > Min then
         Min := RW_Intern (RW_6'First);
      end if;
   ...
   begin
      ...
      if RW_Intern (RW_6'Last) < Max then
         Max := RW_Intern (RW_6'Last);
      end if;
   ...
   -- Das ist eine 1 zu 1 Umsetzung des offiziellen PAP für das Steuerjahr 2004
   ...
   begin
      ...
      RE4LZZ := Cent (RE4) - FVB - ALTE - WFUNDF + HINZUR;
      RE4LZZV := Cent (RE4) - FVB - ALTE;
   end MRE4LZZ;
   ...
   begin
      ...
      ZVE := Euro (ZRE4) - ZTABFB - Euro (VSP); -- Test, ob VSP + ZRE4 keine Nachkommastellen haben!
      MLSTJAHR_2;
      MLSTJAHR_3;
   end MLSTJAHR_1;
   ...
   begin
      if ZVE < 1 then
         ...
         --         UPRUND36;
         --         X := RW_1 (RUND + 18);
         X := Abrunden_2 (RW_Intern (ZVE) / RW_Intern (KZTAB));
      end if;
   end MLSTJAHR_2;
   ...
   procedure MVSP is
   begin
      VSPO := RW_2 (RW_Intern (ZRE4VP) * 0.20); -- prüfen, sollte aber ok sein
   ...
   procedure UP5_6 is
   begin
      -- aha, hat das bmf ein einsehen gehabt:
      X := RW_2 (RW_Intern (ZX) * 1.25); -- prüfen, ob 2. Nachkommastelle auftritt. Dürfte nicht sein.
      UPTAB04;
      ST1 := ST;
      X := RW_2 (RW_Intern (ZX) * 0.75); -- wie oben.
      UPTAB04;
      ST2 := ST;
      DIFF := (ST1 - ST2) * 2;
      MIST := Abrunden_0 (RW_Intern (ZX) * 0.16); -- auf volle EUR abrunden.
      if MIST > DIFF then
         ST := MIST;
      else
         ST := DIFF;
   ...
   begin
      ...
      if ZZX > 26072 then
         ZX := 26072;
         UP5_6;
         ST := Abrunden_0 (RW_Intern (ST) + (RW_Intern (ZZX - 26072)) * 0.45); -- auf volle EUR abrunden.
      else
         ZX := ZZX;
         UP5_6;
         if ZZX > 9228 then
            ...
            ZX := 9228;
            UP5_6;
            HOCH := Abrunden_0 (RW_Intern (ST) + (RW_Intern (ZZX - 9228)) * 0.45); -- auf volle EUR abrunden.
            if HOCH < VERGL then
               ST := HOCH;
            else
               ST := VERGL;
   ...
   begin
      if KENNZ = 1 then
         VSPVOR := RW_2 (RW_Intern (VSPVOR) - RW_Intern (ZRE4VP1) * 0.16);
      else
         VSPVOR := RW_2 (RW_Intern (VSPVOR) - RW_Intern (ZRE4VP) * 0.16);
      end if;
      if VSPVOR < 0.0 then
         VSPVOR := 0.0;
      end if;
      if VSPO > VSPVOR then
         ...
         if VSPREST > RW_2 (VSPMAX1) then
            ...
            if VSPREST > RW_2 (VSPMAX2) then
               VSP := Abrunden_2 (RW_Intern (VSP) + RW_Intern (VSPMAX2));
            else
               VSP := Abrunden_2 (RW_Intern (VSP) + RW_Intern (VSPREST));
            end if;
         else
            VSP := Abrunden_2 (RW_Intern (VSP) + RW_Intern (VSPREST));
         end if;
   ...
   begin
      if SONSTB > 0 then
         ...
         LST2 := Cent (ST) * 100;
         STS := LST2 - LST1;
         SOLZS := Cent (Abrunden_0 (RW_Intern (STS) * 5.5 / 100.0));
         if R > 0 then
            BKS := STS;
         else
            BKS := 0;
   ...
   begin
      if VMT > 0 then
         ...
         SOLZV := Cent (Abrunden_0 (RW_Intern (STV) * 5.5 / 100.0));
         if R > 0 then
            BKV := STV;
         else
            BKV := 0;
   ...
   procedure UPTAB04 is
   begin
      if X < 7665.0 then
         ST := 0;
      elsif X < 12704.0 then
         Y := (X - 7664.0) / 10000.0;
         RW := RW_Intern (Y) * 793.10;
         RW := RW + 1600.0;
         ST := Abrunden_0 (RW_Intern (RW) * RW_Intern (Y)); -- auf volle EUR abrunden.
      elsif X < 52152.0 then
         Y := (X - 12739.0) / 10000.0;
         RW := RW_Intern (Y) * 265.78;
         RW := RW + 2405.0;
         RW := RW_Intern (RW) * RW_Intern (Y);
         ST := Abrunden_0 (RW_Intern (RW) + 1016.0); -- auf volle EUR abrunden.
      else
         ST := Abrunden_0 (RW_Intern (X) * 0.45 - RW_Intern (8845)); -- auf volle EUR abrunden.
      end if;
      ST := ST * Euro (KZTAB);
   end UPTAB04;
